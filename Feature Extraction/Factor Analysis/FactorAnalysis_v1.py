...
def feature_extraction():
    ...
    x_train = x_train[::3, ::3]
    x_test = x_test[::3, ::3]
    x_train = x_train.reshape((22*17, x_train.shape[2])).T
    x_test = x_test.reshape((22*17, x_test.shape[2])).T
    ...
    fa_train = FactorAnalyzer(n_factors=64, rotation="varimax")
    ...
    # Update features based on loadings
    x_trainR = np.dot(x_train, loadings).T
    x_testR = np.dot(x_test, loadings).T

    # Reshape data
    x_trainR = x_trainR.reshape(8, 8, x_trainR.shape[1])
    x_testR = x_testR.reshape(8, 8, x_testR.shape[1])
